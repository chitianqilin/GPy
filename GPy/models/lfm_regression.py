...
class LFMRegression(GP):
    ...
    """
    def __init__(self, X_list, Y_list, kernel=None, likelihoods_list=None, name='GPLFM', kernel_name='lfm'):
        ...
        # Kernel
        if kernel is None:
            kernel = kern.LFM(input_dim=X.shape[1], output_dim=Ny, scale=10*np.ones(Ny), active_dims=None)

        # Likelihood
        likelihood = util.multioutput.build_likelihood(Y_list, self.output_index, likelihoods_list)

        super(LFMRegression, self).__init__(X, Y, kernel, likelihood, inference_method=None,  Y_metadata={'output_index': self.output_index})
