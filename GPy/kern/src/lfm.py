...
def lfmComputeH3(self, gamma1_p, gamma1_m, sigma2, X, X2, preFactor, mode=None, term=None):
    ...
    if not mode:
        ...
        upsilon = [lfmUpsilonMatrix(gamma1_p, sigma2, X, X2), lfmUpsilonMatrix(gamma1_m, sigma2, X, X2)]
        h = preFactor[0] * upsilon[0] + preFactor[1] * upsilon[1]
    return h

def lfmComputeH4(self, gamma1_p, gamma1_m, sigma2, X, preFactor, preExp, mode=None, term=None ):
    ...
    if not mode:
        if not term:
            upsilon = lfmUpsilonVector(gamma1_p, sigma2, X)
        ...
        upsilon = [lfmUpsilonVector(gamma1_p, sigma2, X), lfmUpsilonVector(gamma1_m, sigma2, X)]
        h = upsilon[0] * (preExp[:, 0] / preFactor[0] - preExp[:, 1] / preFactor[1]).T \
            + upsilon[1] * (preExp[:, 1] / preFactor[2] - preExp[:, 0] / preFactor[3]).T
    return h
...
class LFM(Kern):
    ...
    def K_sub_matrix(self, q, X, q2= None, X2=None):
        ...
        if self.omega_isreal[q] and self.omega_isreal[q2]:
            ...
            sK = np.real(
                        lfmComputeH3(gamma1, gamma2, self.scale, X, X2, preConsX, 0, 1)
                        + lfmComputeH3(gamma2, gamma1, self.scale, X2, X, preConsX[1] - preConsX[0], 0, 0).T
                        + lfmComputeH4(gamma1, gamma2, self.scale, X, preGamma, preExp2, 0, 1)
                        + lfmComputeH4(gamma2, gamma1, self.scale, X2, preGamma, preExp1, 0, 0).T
                        )
            ...
            preConsX = 1. / preGamma
            preFactors = np.array([ preConsX[2] - preConsX[1],
                                    preConsX[3] - preConsX[4],
                                    preConsX[3] - preConsX[1],
                                    preConsX[2] - preConsX[4]
                                ])
            ...
            # Actual  computation of the kernel
            sK = (
                    lfmComputeH3(gamma1_p, gamma1_m, self.scale, X, X2, preFactors[1, 2], 1)
                    + lfmComputeH3(gamma2_p, gamma2_m, self.scale, X2, X, preFactors[3, 4], 1).T
                    + lfmComputeH4(gamma1_p, gamma1_m, self.scale, X, preGamma[1, 2, 4, 3], preExp2, 1)
                    + lfmComputeH4(gamma2_p, gamma2_m, self.scale, X2, preGamma[1, 3, 4, 2], preExp1, 1).T
                )
            if self.isNormalised[q]:
                K0 = (self.sensitivity[q] * self.sensitivity[q2]) / (
    ...
    def Kdiag(self, X):
        assert X.shape[1] == 1, 'Input can only have one column'
        slices = index_to_slices(X[:,self.index_dim])
        target = np.zeros((X.shape[0]))
        [
            [np.copyto(target[s], self.Kdiag_sub(q, X[s])) for s in slices_i]
    ...
    def Kdiag_sub(self, q, X):
        ...
        def lfmDiagComputeH4(gamma, sigma2, t, factor, preExp, mode):
            if mode:
                ...
                temp2 = np.multiply(temp, np.conj(preExp)) / factor[1]
                vec = np.multiply(temp, preExp) / factor[0] - 2 * np.real(temp2)
            return vec
        ...
        sk = lfmDiagComputeH3(-gamma_m, sigma2, X, preFactors[0], preExp[:, 1], 1)  \
            ...
            + lfmDiagComputeH4(gamma_m, sigma2, X, [gamma_m(gamma_p + gamma_m)], [preExp[:, 1], preExp[:, 0]], 1)  \
            + lfmDiagComputeH4(gamma_p, sigma2, X, [gamma_p(gamma_p + gamma_m)], preExp, 1)
        if self.isNormalised:
            k0 = self.sensitivity ** 2 / (8 * np.sqrt[1] * self.mass[q] ** 2 * self.omega[q] ** 2)
        else:
            k0 = np.sqrt(np.pi) * self.sigma[q] * self.sensitivity ** 2 / (8 * self.mass[q] ** 2 * self.omega[q] ** 2)
        k = k0 * sk
        return k
    ...
    def _update_gradients(self, q, q2, X, X2=None, dL_dK=None, meanVector=None):
        ...
        assert np.shape(X)[1] == 1 or np.shape(X2)[1] == 1,  'Input can only have one column.'
        assert  self.scale[q] == self.scale[q2], 'Kernels cannot be cross combined if they have different inverse widths.'
        ...
        for ind_theta in np.arange(3):  # Parameter (m, D or C)
            for ind_par in np.arange(2):  # System (1 or 2)
                ...
                if ind_theta ==1: # Gradient wrt D
                    gradThetaM = np.zeros((1, 2))
                    gradThetaAlpha = np.zeros((1, 2))
                ...
                if np.all(np.isreal(omega)):
                    ...
                    if not ind_par:  # ind_par = k
                        matGrad = K0 *  \
                                    (lfmGradientH31(preFactors[0,1], preFactors2[0,1], gradThetaGamma1, gradientUpsilonMatrix[0],
    ...
    def update_gradients_full(self, dL_dK, X, X2=None):
        ...
        if X2 is not None:
            ...
            for j in range(len(slices2)):
                for i in range(len(slices)):
                    for l in range(len(slices2[j])):
                        for k in range(len(slices[i])):
                            [g1,g2]=self._update_gradients(i, j, X[slices[i][k], :], X2[slices2[j][l], :],
        ...
        self.scale.gradient += (g[3]) * (-2 / np.power(self.scale, 2))
        self.mass.gradient += g[0]
        self.spring.gradient += g[1]
        self.damper.gradient += g[2]
        self.sensitivity.gradient += g[4]
    ...
    def update_gradients_diag(self, dL_dKdiag, X):
        self.reset_gradients()
        slices = index_to_slices(X[:, self.index_dim])
        [[self._update_gradients_diag_wrapper(self.covariance[i][i]['kern'], dL_dKdiag[slices[i][k]],
                                              X[slices[i][k], :]) for k in range(len(slices[i]))] for i in
         range(len(slices))]
    ...
    def gradients_X(self, dL_dK, X, X2=None):
        ...
        if X2 is not None:
            slices2 = index_to_slices(X2[:, self.index_dim])
            [[[[target.__setitem__((slices[i][k]), target[slices[i][k], :] + self.covariance[i][j]['gradients_X'](
                dL_dK[slices[i][k], slices2[j][l]], X[slices[i][k], :], X2[slices2[j][l], :])) for k in
                range(len(slices[i]))] for l in range(len(slices2[j]))] for i in range(len(slices))] for j in
             range(len(slices2))]
        else:
            [[[[target.__setitem__((slices[i][k]), target[slices[i][k], :] + self.covariance[i][j]['gradients_X'](
                dL_dK[slices[i][k], slices[j][l]], X[slices[i][k], :],
                (None if (i == j and k == l) else X[slices[j][l], :]))) for k in range(len(slices[i]))] for l in
               range(len(slices[j]))] for i in range(len(slices))] for j in range(len(slices))]
        return target